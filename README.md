...
- **The 30 gallery photos are placeholders** (soft gradient tiles labelled "Sample photo 01" etc.) — real photography hasn't been dropped in yet. To swap them in: replace the files in `images/baby/`, `images/cakesmash/`, `images/family/`, keeping the same names (`01.jpg` through `10.jpg` in each folder) and roughly the same 4:5 portrait shape, and the galleries pick them up automatically — no HTML changes needed. Fewer than 10 real photos for a session is fine; just delete the unused numbered files from that folder and update the `<button>` blocks in the matching `gallery-*.html` to match.
